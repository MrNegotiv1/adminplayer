...
temp_root = tk.Tk()
temp_root.withdraw()
user_token = simpledialog.askstring("Авторизация", "Введите ключ (тгк @nerest_skripts):")
if not user_token:
    messagebox.showinfo("Выход", "Ключ не введён.")
    exit()

tokens = load_tokens()
if not is_token_valid(user_token, tokens):
    messagebox.showerror("Ошибка", "Неверный или просроченный ключ.\nтгк @nerest_skripts")
    exit()
...
def build_pro_tab():
    ...
    if user_license != "pro":
        ...
        # Слайдер скорости
        ctk.CTkLabel(frame, text="Скорость:").pack(pady=(10, 0))
